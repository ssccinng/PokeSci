...
namespace PSReplayAnalysis
{
    public static class ExporttoTrainData
    {
        ...
        public static int[] ExportBattleTurn(BattleTurn turn, int p)
        { 
            var ss = new int[4][];
            // 可能要有turn的观念
            ...
            ss[p == 1 ? 1 : 2] = team1Space.Concat(side1Space).Concat(side1Field).ToArray();
            ss[p == 1 ? 2 : 1] = team2Space.Concat(side2Space).Concat(side2Field).ToArray();
