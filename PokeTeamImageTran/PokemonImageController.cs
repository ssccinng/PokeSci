...
public class PokemonImageController : ControllerBase
{
    ...
    PokeOCRSV.PokeRegion[] _pokeRegions1 = {
    ...
    //public PokemonImageController()
    //{
    //    foreach (var item in _pokeRegions1)
    //    {
    //        item.RectangleTera.x
    //    }
    //}
    ...
    public async Task<object> GetNew(string lang)
    {
        var bytes = new byte[(int)Request.ContentLength];
        await Request.Body.CopyToAsync(new MemoryStream(bytes));

        var rect = DelectUtil.Delect(bytes);
        rect.Reverse();

        for (int i = 0; i < rect.Count; ++i)
        {
            Console.WriteLine($"{rect[i].X} {rect[i].Y} {rect[i].Width} {rect[i].Height}");
        }
    ...
    public async Task<object> Get1New(string lang)
    {
        var bytes = new byte[(int)Request.ContentLength];
        await Request.Body.CopyToAsync(new MemoryStream(bytes));
