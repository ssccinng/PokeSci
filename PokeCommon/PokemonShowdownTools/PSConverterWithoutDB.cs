...
namespace PokeCommon.PokemonShowdownTools
{
    public class PSConverterWithoutDB
    {
        ...
        public static async ValueTask<GamePokemon> ConvertFromPsOneLineAsync(string oneLine)
        {
            ...
            {
                gamePokemon.Item = await PokemonToolsWithoutDB.GetItemAsync(data[2]);
            }

            // 3: 特性
            if (!string.IsNullOrEmpty(data[3]))
            {
                gamePokemon.Ability = await PokemonToolsWithoutDB.GetAbilityAsync(data[3]);
            }
            ...
            {
                gamePokemon.Nature = await PokemonToolsWithoutDB.GetNatureAsync(data[5]);
            }
            ...
            if (!string.IsNullOrEmpty(data[11]))
            {
                ...
                {
                    gamePokemon.TreaType = await PokemonToolsWithoutDB.GetTypeAsync(teraData[5]);
                }
            }
